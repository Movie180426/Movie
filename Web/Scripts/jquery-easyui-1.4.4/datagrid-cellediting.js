(function ($) {
    $.extend($.fn.datagrid.defaults, {
        ...
        endclickToNext: true,   //启用编辑完成后自动编辑下一个
        navHandler: {   //按键处理
            '37': function (e) {
    ...
    function editCell(target, param) {
        ...
        if (col.editor) {
            ...
            if (input) {
                dg.datagrid('gotoCell', param);
                setTimeout(function () {
                    input.unbind('.cellediting').bind('keydown.cellediting',
                    function (e) {
                        if (e.keyCode == 13) {
    ...
    function enableCellSelecting(target) {
        ...
        dc.body1.add(dc.body2).unbind('.cellediting').bind('click.cellediting',
        function (e) {
